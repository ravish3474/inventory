...
$n_row = $start_from+1;
$sql_detail = "SELECT forecast_detail.*,forecast_head.is_produced FROM forecast_detail LEFT JOIN forecast_head ON forecast_head.forecast_id=forecast_detail.forecast_id WHERE forecast_order LIKE '%$code%' OR forecast_detail_color LIKE '%$code%' ORDER BY forecast_head.forecast_date DESC LIMIT $start_from, $records_per_page";
